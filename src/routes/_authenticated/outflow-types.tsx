...
import { Plus, Edit, Trash2, Tag, ArrowLeft, Sparkles, Lock } from "lucide-react";
              ...
              >
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                  <CardTitle className="text-sm font-medium flex items-center gap-3">
                    <span className="text-2xl p-2 bg-background rounded-lg shadow-sm">{type.emoji}</span>
          ...
          {outflowTypes ? (
            customTypes.map((type, index) => (
              <Card 
                ...
                style={{ animationDelay: `${(builtInTypes.length + index) * 30}ms` }}
              >
                {/* Color accent */}
                <div 
            ...
            <Card className="col-span-full border-dashed animate-fade-in">
              <CardContent className="flex flex-col items-center justify-center py-12">
                ...
                <p className="text-muted-foreground text-center mb-6 max-w-sm">
                  Create custom categories to better organize your transactions beyond the built-in options.
                </p>
                <AddOutflowTypeDialog
                  open={showAddDialog}
                  onOpenChange={setShowAddDialog}
