...
describe("Users API", () => {
  ...
  describe("createUser", () => {
    it("should create user with default outflow types", async () => {
      ...
      const outflowTypes = await asUser.query(
        api.outflowTypes.listOutflowTypes
      );
    ...
    it("should not duplicate data on second call", async () => {
      ...
      const outflowTypes2 = await asUser.query(
        api.outflowTypes.listOutflowTypes
      );
  ...
  describe("deleteAllUserData", () => {
    it("should throw error when not authenticated", async () => {
      ...
      await expect(t.mutation(api.users.deleteAllUserData)).rejects.toThrow(
        "Not authenticated"
      );
    });
  ...
  describe("seedOutflowTypes", () => {
    it("should throw error when not authenticated", async () => {
      ...
      await expect(t.mutation(api.users.seedOutflowTypes)).rejects.toThrow(
        "Not authenticated"
      );
    });
    ...
    it("should not duplicate types on second call", async () => {
      ...
      await asUser.mutation(api.users.seedOutflowTypes);
      const firstCount = (await asUser.query(
        api.outflowTypes.listOutflowTypes
      ))!.length;

      await asUser.mutation(api.users.seedOutflowTypes);
      const secondCount = (await asUser.query(
        api.outflowTypes.listOutflowTypes
      ))!.length;
